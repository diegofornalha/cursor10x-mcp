...
# Script de monitoramento para MCP Turso (Claude Code)
# Uso: ./monitor-turso-claude.sh

echo "📊 Monitor MCP Turso (Claude Code)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
show_stats() {
    clear
    echo -e "${BLUE}📊 MCP Turso Monitor (Claude Code) - $(date)${NC}"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    ...
    # Verificar configuração no Claude Code
    echo -e "${BLUE}🔧 Configuração Claude Code:${NC}"
    if claude mcp list 2>/dev/null | grep -q "turso"; then
        echo -e "  ${GREEN}✅ Configurado no Claude Code${NC}"
        # Mostrar status
        TURSO_STATUS=$(claude mcp list 2>/dev/null | grep "turso" | grep -o "✓ Connected" || echo "")
        if [ -n "$TURSO_STATUS" ]; then
            echo -e "  ${GREEN}✅ Status: Conectado${NC}"
        else
            echo -e "  ${RED}❌ Status: Desconectado${NC}"
        fi
    else
        echo -e "  ${YELLOW}⚠️  Não configurado no Claude Code${NC}"
    fi
    echo ""
